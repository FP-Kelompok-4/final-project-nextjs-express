...
export const toPropertyRoomPriceRes = (properties: PropertyRoomPrice[]) => {
  return properties.map(({ id, name, description, image, location, rooms }) => {
    return {
      id,
      name,
      description,
      location,
      image,
      price: rooms.map((v) => v.roomPrices?.price).filter((price) => price)[0],
    };
  });
};
